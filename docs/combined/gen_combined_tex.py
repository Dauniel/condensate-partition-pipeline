...
\usepackage{titlesec}
\usepackage{fancyhdr}
\usepackage[hidelinks]{hyperref}      % clickable links, black, no box/underline
...
\begin{center}
\includegraphics[width=0.78\textwidth]{gui_main.png}\\
{\small The Single-File tab configured for a JABr run: a multi-channel TIF loaded, Construct = JABr.}
\end{center}
\begin{center}
\includegraphics[width=0.78\textwidth]{gui_done.png}\\
{\small After the run: the Output Log reports the calibrated partition coefficient and the status reads Done.}
\end{center}

\subsubsection*{Single-channel mode}
A third input mode, \textbf{Single channel}, segments just one image --- a
condensate \emph{or} a nuclei image --- when you do not have both channels. Pick
the type from the drop-down; nuclei use Cellpose \code{cyto3} and condensate uses
the same detector the full pipeline would (for \code{JABr}, \code{blob\_log}, with
the intra-nuclear gate disabled so every detection is kept). It outputs masks,
object count, volumes, and per-slice measurements --- but \textbf{no partition
coefficient}, which requires both channels.
\begin{center}
\includegraphics[width=0.78\textwidth]{gui_single.png}\\
{\small Single-channel mode: segment one condensate (or nuclei) image on its own.}
\end{center}
...
Separate channels: \code{-{}-nuc nuclei.tif -{}-cond condensate.tif}. Single-channel
segmentation (one image, no PC): \code{-{}-single condensate.tif -{}-channel condensate}
(or \code{-{}-channel nuclei}). Physical volumes in $\mu m^3$: add
\code{-{}-voxel-xy 0.065 -{}-voxel-z 0.3}. Force CPU: \code{-{}-no-gpu}. For many files
(and optional comparison vs a manual reference CSV), use the GUI \textbf{Batch} tab.
